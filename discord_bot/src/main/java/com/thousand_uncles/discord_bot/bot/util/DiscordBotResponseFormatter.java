...
public class DiscordBotResponseFormatter {
    ...
    public static String mapRecordToMessageContent(ManualIndexedMapRecordEntry requestedMap) {
        String response = "";
        response += "Current WR: " + RecordFormatter.BigDecimalToString(requestedMap.getCurr_wr_seconds()) + "\n";
        response += "Previous WR: " + RecordFormatter.BigDecimalToString(requestedMap.getPrev_wr_seconds()) + "\n";
        if (requestedMap.getProof_img_2_link() == null) {
            ...
            try {
                response += "Stage 1 time: " + RecordFormatter.BigDecimalToString(requestedMap.getStage_1_time_seconds()) + "\n";
                response += "Stage 2 time: " + RecordFormatter.BigDecimalToString(requestedMap.getStage_2_time_seconds()) + "\n";
                response += "Stage 3 time: " + RecordFormatter.BigDecimalToString(requestedMap.getStage_3_time_seconds()) + "\n";
            } catch (Exception e) {
    ...
    public static String NumberToString (BigDecimal timeNumber) {
        String timeString;
        BigDecimal minutes = timeNumber.divide(BigDecimal.valueOf(60),0, RoundingMode.FLOOR);
