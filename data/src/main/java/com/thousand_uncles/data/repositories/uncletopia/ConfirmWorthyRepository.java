...
@Repository
public interface ConfirmWorthyRepository extends JpaRepository<ConfirmWorthyMapRecordEntry, Integer> {

    @Query("SELECT m FROM ConfirmWorthyMapRecordEntry m WHERE m.map_id = :map_id AND m.category = :category")
    ConfirmWorthyMapRecordEntry findMapByCategory(@Param("map_id") Integer map_id, @Param("category") String category);

    @SuppressWarnings("unused")
    @Modifying
    @Query("DELETE FROM ConfirmWorthyMapRecordEntry m WHERE m.map_id = :map_id AND m.category = :category")
    int removeMapByCategory(@Param("map_id") Integer map_id, @Param("category") String category);

    @SuppressWarnings("unused")
    @Modifying
    @Query("INSERT INTO ConfirmWorthyMapRecordEntry (category, map_id, map_name, curr_wr_seconds, prev_wr_seconds, proof_img_1_link, proof_img_2_link, proof_img_3_link, proof_vid_link, stage_1_time_seconds, stage_2_time_seconds, stage_3_time_seconds, additional) " +
            "VALUES " +
