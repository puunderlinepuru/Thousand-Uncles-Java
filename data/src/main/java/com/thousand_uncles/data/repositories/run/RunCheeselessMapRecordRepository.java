...
@Repository
public interface RunCheeselessMapRecordRepository extends JpaRepository<RunCheeselessMapRecordEntry, Integer> {
    @Query("SELECT m FROM RunCheeselessMapRecordEntry m WHERE LOWER(m.map_name) LIKE LOWER(CONCAT('%', :name, '%'))")
